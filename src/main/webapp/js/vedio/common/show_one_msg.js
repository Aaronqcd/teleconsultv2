...
function showMsgTime(time, $item, isBefore){
    var d = new Date();
    d.setTime(time * 1000)
    var formatTime = fillZero(d.getHours()) + ":" + fillZero(d.getMinutes()) + ":" + fillZero(d.getSeconds())

    var $msgItemContainer = $("<div>").addClass("meeting-chat-info system").attr("msg-time", time)
    $("<div>").addClass("meeting-system-info-text").appendTo($msgItemContainer).html(formatTime)

    if(isBefore){
        $item.before($msgItemContainer)
    }else{
        $item.after($msgItemContainer)
    }
}

function assertShowMsgTime(time){
    var preMsgTime = parseInt($(".meeting-chat-content").children(".meeting-chat-info:last").attr("msg-time"))
    if (time - preMsgTime > 120){
        showMsgTime(time, $(".meeting-chat-content").children(".meeting-chat-info:last"), false)
    }
}

//渲染新增消息
function showNewMsg(msgContent, userName, userAvatar, isSelf, prepend, msgTime){
    var  reg = /<[^>]+>/g;

    if (reg.test(msgContent)) {
        $msgItemContainer = $("<div>").addClass("meeting-chat-info").addClass(isSelf ? "target" : "other").attr("msg-time", msgTime)
        $userDiv = $("<div>")

        $userName = $("<span>").addClass("meeting-chat-name").text(userName)
        $headImg = $("<span>").addClass("meeting-chat-img").append($("<img>").attr("src", userAvatar))

        if(isSelf){
            $userDiv.append($userName)
            $userDiv.append($headImg)
        }else{
            $userDiv.append($headImg)
            $userDiv.append($userName)
        }

        $msgItemContainer.append($userDiv)

        // var classes = $(msgContent).attr("class")


        $msgContentDiv = $("<div>").addClass("meeting-chat-text").appendTo($msgItemContainer).attr("time", msgTime).attr("from-user", userName)
        $msgContentDiv.html((msgContent).replace(/&amp;/g, "&"))
	
        if(prepend){
            $(".meeting-chat-content").prepend($msgItemContainer)
        }else{

            assertShowMsgTime(msgTime)
            $(".meeting-chat-content").append($msgItemContainer)
        }
     }else{
        try {
            ...
            if(msgObj['toUserID'] && msgObj['msgType']){
                if( msgObj['toUserID'] == meetingConfig.userID ){
                    var callbackList = IMUtils.__data[msgObj['msgType']]['to']
                    console.log("收到自定义消息类型", msgObj['msgType'], msgObj, "接收方执行")
                }else if (msgObj['fromUserID'] != meetingConfig.userID){
                    if(!msgObj['msgData']['isExit']){
                        var callbackList = IMUtils.__data[msgObj['msgType']]['other']
                        console.log("收到自定义消息类型", msgObj['msgType'], msgObj, "其他人员执行")
                    }
                }
                if(callbackList ){
                    callbackList.forEach(function(callback){
                        invokeFunc(callback, msgObj['msgData'])
                    })
                }
            }
            /*if (msgObj['msgType']) {
                if( msgObj['msgType'] == "openVideoCollapse" ){
                    var callbackList = VIDEOCOLLAPSEUtils.__data[msgObj['msgType']]['open']
                    console.log("收到自定义消息类型", msgObj['msgType'], msgObj, "接收方执行")
                }else if (msgObj['msgType'] == "closeVideoCollapse"){
                    var callbackList = VIDEOCOLLAPSEUtils.__data[msgObj['msgType']]['close']
                    console.log("收到自定义消息类型", msgObj['msgType'], msgObj, "其他人员执行")
                }
                if(callbackList ){
                    callbackList.forEach(function(callback){
                        invokeFunc(callback, msgObj)
                    })
                }
            }*/
        }catch(err){
			console.error(err)
            console.log("JSON.parse(msgContent) err msgContent: ", msgContent)
        }
    }

}

function getUserHeadurl(userAccount, callback){

    var userInfo = userInfoMap[userAccount];
    invokeFunc(callback, userInfo ? userInfo.userName : "", userInfo ? userInfo.headImg : meetingConfig.defaultUserAvatar)

}


function showNormalMsg(fromAccount, prepend, msg, isSelfSend){
    //只显示普通消息

    getUserHeadurl(fromAccount, function(userAccountNick, userAccountHeadurl){
        var msgContent = convertMsgtoHtml(msg);

        //消息列表
        var msgflow = $(".meeting-chat-content")[0];
        if (prepend) {
            //300ms后,等待图片加载完，滚动条自动滚动到底部
            showNewMsg(msgContent, userAccountNick, userAccountHeadurl, isSelfSend, prepend, msg.time)
...
function addMsg(msg, prepend) {
    ...
    switch (subType) {
        ...
        case webim.GROUP_MSG_SUB_TYPE.COMMON: //群普通消息
            ...
            showNormalMsg(fromAccount, prepend, msg, isSelfSend)
...
function convertMsgtoHtml(msg) {
    var html = "",
        elems, elem, type, content, result;
    var $html = $("<div>")
            ...
            for (var m in groupInfoList) {
                ...
                switch (type) {
                    ...
                    case webim.GROUP_TIP_MODIFY_GROUP_INFO_TYPE.OWNER:
                        text += "群主为" + value + "; ";
                        console.log("群主变更：", value)
                        if (value == meetingConfig.userID){
                            changeToGroupOwner()
                            EventUtils.fire("changeGroupOwnerNewPresenterCallback")
                        }
                        break;
...
function showSystemInfo(info, msgTime, prepend){
    info = "<span style='color: red;'>"+info+"</span>"
    var $msgItemContainer = $("<div>").addClass("meeting-chat-info system").attr("msg-time", msgTime)
    $("<div>").addClass("meeting-system-info-text").appendTo($msgItemContainer).html(info)
