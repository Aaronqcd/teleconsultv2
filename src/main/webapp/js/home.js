/**
 * home.js 主页面操作定义 by 青阳龙野 该文件为home.html的操作定义文件，包含了页面上的主要操作实现。
 */
var layer;
...
var checkFilesTip="提示：您还未选择任何文件，请先选中一些文件后再执行本操作。<br />";// 选取文件提示
var winHeight;// 窗口高度
...
$(function() {
	...
	document.ondrop = function(e) {
		...
		if (folderView.authList != null) {
			if (checkAuth(folderView.authList, "U")) {// 如果有上传权限且未进行其他上传
				if(isUpLoading){
					layer.alert("提示：您正在执行另一项上传任务，请在上传窗口关闭后再试。");
				}else{
					if (!(window.ActiveXObject||"ActiveXObject" in window)){// 判断是否为IE
						...
						if(testFile){
							...
							layer.alert("提示：您拖入的文件中包含了一个或多个文件夹，无法进行上传。");
						}
					}else{
						layer.alert("提示：IE浏览器不支持拖拽上传。您可以使用现代浏览器或将浏览模式切换为“极速模式”来体验该功能。");
					}
				}
			}else{
				layer.alert("提示：您不具备上传权限，无法上传文件。");
			}
		}else{
			layer.alert("提示：您不具备上传权限，无法上传文件。");
		}
	...
	$('#folderInfoModal').on('show.bs.modal', function(e) {
		...
		$("#fim_name").text(f.folderName == 'root' ? '我的云盘' : f.folderName);
...
// 全局请求失败提示
function doAlert(){
	layer.alert("错误：无法连接到服务器，请检查您的网络连接或查看服务器运行状态。");
}
	...
	$.ajax({
		...
		success : function(result) {
			...
			if(result == "ERROR"){
				...
				if(folderView.folder.folderName == 'root'){
					$("#navHeading").hide();
				}else{
					$("#navHeading").show();
				}
...
function showParentList(folderView) {
	...
	if(folderView.parentList.length>0){
		$.each(folderView.parentList, function(n, val) {
			$("#parentFolderList").append("<li><a href='javascript:void(0);' onclick='entryFolder("+'"' + val.folderId +'"'+")'><span class='glyphicon glyphicon-folder-close'></span>"+(val.folderName == 'root' ? '我的云盘' : val.folderName)+"</a></li>");
			$('#filetablenav').append("<a  href='javascript:void(0);' style='margin-left: 0px; color:#666;'  class='navbar-text' onclick='entryFolder("+'"' + val.folderId +'"'+")'><span class='glyphicon glyphicon-folder-close'></span>&nbsp;"+(val.folderName == "root" ? "我的云盘" : val.folderName)+"</a> <span class='navbar-text' style='margin: 15px 10px 0px 0px'>&gt;</span>");
		});
		$('#filetablenav').append("<a  href='javascript:void(0);'style='margin-left: 0px; color:#666;' class='navbar-text' onclick='entryFolder("+'"' + f.folderId +'"'+")'><span class='glyphicon glyphicon-folder-close'></span>&nbsp;"+(f.folderName)+"</a>");
	}else{
		$("#parentFolderList").html("<li class='disabled'><a>无</a></li>");
		$('#filetablenav').html("");
	}
	$("#currentFolderName").text(f.folderName == 'root' ? '我的云盘' : f.folderName);
...
function showAccountView(folderView) {
	...
	if(folderView.folder.folderName === 'root'){
		$('#fileListDropDown li.divider').hide();
		$('#createFolderButtonLi').hide();
		$('#uploadFileButtonLi').hide();
	}else{
		$('#createFolderButtonLi').show();
		$('#uploadFileButtonLi').show();
	}
	...
	if (authList != null) {
		...
		if (checkAuth(authList, "L")) {
			$("#packageDownloadBox").html("<button class='navbar-btn layui-btn layui-btn-sm layui-btn-radius tc-bg-main' onclick='showDownloadAllCheckedModel()' style='height: 30px;line-height: 30px;padding: 0 10px;font-size: 12px;'> 打包下载</button>");
		}else{
...
function showFolderTable(folderView) {
	...
	$.each(
					...
					function(n, f) {
						...
						var folderRow = "<tr id='"+f.folderId+"'  class='filerow' iskfolder='true' ><td class='hiddenColumn'><input class='items' onchange='switchCheck(this)' type='checkbox' style='margin: 4px 0 0 6px;' /></td><td><button onclick='entryFolder("
								+ '"' + f.folderId + '"'
								+ ")' class='btn btn-link btn-xs'><span class='glyphicon glyphicon-folder-close'></span>&nbsp;"
								+ f.folderName + "</button></td><td class='hiddenColumn'>"
								+ f.folderCreationDate + "</td><td>--</td><td class='hiddenColumn'>"
								+ f.folderCreator + "</td><td style='padding: 8px 8px 8px 5px;'>";
						if (aD) {
							folderRow = folderRow
									...
									+ f.folderName
									+ '"'
									+ ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 删除</button>";
						}
						if (aR) {
							folderRow = folderRow
									...
									+ '",'
									+ f.folderConstraint
									+ ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 重命名</button>";
						}
						if (aO) {
							folderRow = folderRow
									...
									+ f.folderId
									+ '"'
									+ ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 定位</button>";
						}
						if (!aR && !aD && !aO) {
							folderRow = folderRow + "--";
						}
	...
	$.each(
			...
			function(n, fi) {
				...
				var fileRow = "<tr id=" + fi.fileId + "  id='" + fi.fileId
						+ "' class='filerow'><td class='hiddenColumn'><input class='items' onchange='switchCheck(this)' type='checkbox' style='margin: 2px 0 0 5px;'  /></td><td>" + fi.fileName
						+ "</td><td class='hiddenColumn'>" + fi.fileCreationDate + "</td>";
				fileRow=fileRow+"<td>" + bytesToSize(fi.fileSize) + "</td>";
				fileRow=fileRow +"<td class='hiddenColumn'>" + fi.fileCreator + "</td><td style='padding: 8px 8px 8px 5px;'>";
				if (aL) {
					fileRow = fileRow
							+ "<button onclick='dodownload("+ '"' + fi.fileId + '"' + ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 下载</button>";
					...
					case "flv":
						...
						+ fi.fileId
						+ '"'
						+ ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 播放</button>";
						break;
					case "pdf":
						fileRow = fileRow
						...
						+ fi.fileId
						+ '"'
						+ ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 预览</button>";
						break;
					case "jpg":
					case "jpeg":
					case "gif":
					...
					case "bmp":
						...
						+ fi.fileId
						+ '"'
						+ ")' class='layui-btn layui-btn-sm layui-btn-radius tc-bg-main'> 查看</button>";
						break;
					case "mp3":
					case "wav":
					case "ogg":
...
function showDeleteFolderModel(folderId, folderName) {
	$('#deleteFolderBox')
			.html(
					"<button id='dmbutton' type='button' class='btn btn-danger' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='deleteFolder("
							+ '"' + folderId + '"' + ")'>删除</button>");
...
function showRenameFolderModel(folderId, folderName, type) {
	$("#renameFolderBox").html(
			"<button type='button' class='btn btn-primary' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='renameFolder("
					+ '"' + folderId + '"' + ")'>修改</button>");
		...
		$.ajax({
			...
			success : function(result) {
				if (result == "mustLogin") {
					...
					if (result == "noAuthorized") {
						showRFolderAlert("提示：您的操作未被授权，重命名失败。");
					} else if (result == "errorParameter") {
						showRFolderAlert("提示：参数不正确，重命名失败，请刷新后重试。");
					} else if (result == "nameOccupied") {
						...
						showRFolderAlert("提示：出现意外错误，可能未能重命名文件夹，请刷新后重试。");
					}
				}
			},
			error : function() {
...
function checkUploadFile() {
	...
	if(isUpLoading==false){
		if(fs!=null&&fs.length>0){
			...
			for (var i = 0; i < fs.length; i++) {
				var fileName = fs[i].name.replace(/^.+?\\([^\\]+?)?$/gi, "$1");
			...
			$.ajax({
				...
				success : function(result) {
					if (result == "mustLogin") {
						...
						} else if (result == "fileTypeError") {
							showUploadFileAlert("不支持上传该类型附件！！！文档格式不能使用：jsp，php，asp");
						} else {
...
function doupload(count) {
	...
	if (uploadfile != null) {
		...
		if(uploadfile.size > maxUploadSize){
			showUploadFileAlert("提示：文件[" + uploadfile.name+ "]太大，超过"+maxSizeStr+"，无法上传");
			$("#uls_" + count).text("[失败]");
			$('#umbutton').hide();
			return;
		}
		...
		xhr.onloadend = function() {
			if (xhr.status === 200) {
				...
				if (result == "uploadsuccess") {
					...
					$("#uls_" + count).text("[失败]");
					$('#umbutton').hide();
				} else if (result == "uploadlimiterror") {
					showUploadFileAlert("提示：云盘容量不足,文件【" + fname
							+ "】上传失败,上传被中断。");
					$("#uls_" + count).text("[失败]");
					$('#umbutton').hide();
				} else {
...
function showDownloadModel(fileId, fileName) {
	...
	$("#downloadFileBox")
			.html(
					"<button id='dlmbutton' type='button' class='btn btn-primary' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='dodownload("
							+ '"' + fileId + '"' + ")'>开始下载</button>");
	...
	$('#deleteFileBox')
			.html(
					"<button id='dfmbutton' type='button' class='btn btn-danger' onclick='deleteFile("
							+ '"' + fileId + '"' + ")' style='border-color: #06b4bf!important;background-color: #06b4bf!important;'>删除</button>");
...
function showRenameFileModel(fileId, fileName) {
	...
	$("#renameFileBox").html(
			"<button type='button' class='btn btn-primary' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='renameFile("
					+ '"' + fileId + '"' + ")'>修改</button>");
...
function pdfView(filePath) {
	window.open("pdfview/web/viewer.html?file=/teleconsult/diskResourceController/getPdfView/" + filePath);
}

// 预览Docx文档
function docxView(fileId){
	window.open("pdfview/web/viewer.html?file=/teleconsult/diskResourceController/getWordView/" + fileId);
}

// 预览TXT文档
function txtView(fileId){
	window.open("pdfview/web/viewer.html?file=/teleconsult/diskResourceController/getTxtView/" + fileId);
}
	...
	$.ajax({
		...
		success : function(result) {
			if (result != "ERROR") {
				...
				layer.alert("错误：无法定位要预览的文件或该操作未被授权。");
			}
		},
		error : function() {
			layer.alert("错误：请求失败，请刷新重试。");
		}
	});
}
...
function showDownloadAllCheckedModel() {
	...
	if (faf.size == 0) {
		...
		$("#downloadAllCheckedBox")
				.html(
						"<button id='dclmbutton' type='button' class='btn btn-primary' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='downloadAllChecked()'>开始下载</button>");
...
function showDeleteAllCheckedModel() {
	...
	if (faf.size == 0) {
		...
		$('#deleteFileBox')
				.html(
						"<button id='dfmbutton' type='button' class='btn btn-danger' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='deleteAllChecked()'>全部删除</button>");
...
function startMoveFile(){
	if($("#cutSignTx").hasClass("cuted")&&checkedMovefiles!==undefined){
		...
		$('#moveFilesBox').html("<button id='dmvfbutton' type='button' class='btn btn-danger' style='border-color: #06b4bf!important;background-color: #06b4bf!important;' onclick='doMoveFiles()'>全部移动</button>");
		...
		if (checkedMovefiles==undefined||checkedMovefiles.size == 0) {
			...
			$("#cutSignTx").html("粘贴("+checkedMovefiles.size+")");
			$("#cutSignTx").addClass("cuted");
		}
	}
}
		...
		success : function(result) {
			if (result == "mustLogin") {
				...
				if (result == "noAuthorized") {
					...
					$('#moveFilesMessage').text("提示：不能将文件夹移动到其自身或子目录下，未能全部移动文件");
					$("#moveFilesBox").hide();
					$("#dmvfbutton").attr('disabled', false);
...
function selectInThisPath(keyworld){
	try{
		...
		layer.alert("错误：搜索关键字有误。请在特殊符号（例如“*”）前加上“\\”进行转义。");
	}
}
...
// 普通复选框选中
function switchCheck(ele){
	//console.log(ele);
	var chkObj = $(ele);
	var chk = chkObj.is(':checked');
	//console.log(chk);
	//console.log(chkObj.parent().parent()[0]);
	if(chk){
		chkObj.parent().parent()[0].className += ' info';
    }else{
    	chkObj.parent().parent().removeClass('info');
    }
}
