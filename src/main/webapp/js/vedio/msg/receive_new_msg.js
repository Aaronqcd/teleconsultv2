...
window.selSess = null
window.headUrl = null
window.friendHeadUrl = null
window.groupHeadUrl = null
...
function onMsgNotify(newMsgList, isHistoryMsg) {
    console.info("收到新消息", newMsgList);
    var sess, newMsg;
    //获取所有聊天会话
        ...
        // if (newMsg.getSession().id() == selToID) { //为当前聊天对象的消息
            ...
            try {
			   ...
			   if(msgObj['toUserID'] && msgObj['msgType']){
                ...
                if (isHistoryMsg){
                    if(webim.GROUP_MSG_SUB_TYPE.COMMON == newMsg.subType){
                        addMsg(newMsg, true);
                    }
                }else{
                    addMsg(newMsg);
