 ...
 */
package asr.realtime.model.request;

import static asr.realtime.config.AsrInternalConfig.REAL_ASR_URL;
import static asr.realtime.config.AsrInternalConfig.SIGN_URL;
...
import asr.realtime.config.AsrGlobelConfig;
import asr.realtime.config.AsrInternalConfig;
import asr.realtime.config.AsrPersonalConfig;
import asr.realtime.model.enums.EngineModelType;
import asr.realtime.model.enums.ResponseEncode;
import asr.realtime.model.enums.ReturnType;
import asr.realtime.model.enums.VoiceFormat;
import asr.realtime.utils.StringTools;

/**
 * 通用的基础请求对象，可作为实时、离线语音、TTS等请求对象的父类。
 * 
 * @author iantang
 * @version 1.0
 */
public class RasrBaseRequest {
	...
	private String engineModelTtype = AsrPersonalConfig.engineModelType;
	...
	protected TreeMap<String, Object> initTreeMap() {
		...
		treeMap.put("engine_model_type", this.engineModelTtype);
	...
	public String getEngineModelTtype() {
	...
	public void setEngineModelTtype(String engineModelTtype) {
	 ...
	 * 手工初始化。目前暂未用到。
	 */
	public void manualInit(String engineModelType, ResponseEncode responseEncode, VoiceFormat voiceFormat,
