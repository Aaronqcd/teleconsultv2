 ...
 */
package asr.realtime.http.base;

import asr.realtime.http.base.HttpPoolingManager;
...
import asr.realtime.model.request.RasrBaseRequest;
import asr.realtime.model.response.VoiceResponse;
import asr.realtime.model.response.VoiceResponseParser;
import asr.realtime.utils.SignBuilder;
...
public class HttpRequester {
	...
	public HttpRequester() {
		if (AsrGlobelConfig.USE_CUSTOM_CONNECTION_POOL) {
			...
			this.requestConfig = RequestConfig.custom().setConnectTimeout(1500).setSocketTimeout(300000).build();
		}
	}
	...
	public VoiceResponse sendAndParse(RasrBaseRequest rasrBasesRequest, byte[] dataPacket) {
		...
		if (responseStr != null && responseStr.contains("code") && responseStr.contains("seq")) {
			...
			voiceResponse.setCode(-1);
			voiceResponse.setMessage("Unexpected http response: " + responseStr);
	...
	public Object send(String serverUrl, String sign, byte[] dataPacket) {
		...
		response = retryGetResponse(serverUrl, sign, dataPacket);
		if (response == null) {
			return null;
		}
		try {
	 ...
	 */
	private CloseableHttpResponse retryGetResponse(String serverUrl, String sign, byte[] dataPacket) {
		CloseableHttpResponse response = null;
	...
	private CloseableHttpResponse excuteOnce(HttpPost hpost) {
		try {
			...
			if (e.getMessage().indexOf("connect timed out") >= 0) {
				System.err.println("http connect timed out.");
