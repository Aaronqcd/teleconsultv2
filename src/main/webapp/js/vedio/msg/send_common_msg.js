...
function replace_em(str){
	...
	str = str.replace(/\[([0-9]*)\]/g,'<img src="qqFace/arclist/$1.gif" border="0" />');
...
function onSendMsg(msgContent, callbackList) {
	selToID = meetingConfig['selToID']
    if (!selToID) {
...
//		msgContent = replace_em(msgContent);
	if(typeof(msgContent) == 'object' ){
		...
		var msg = new webim.Msg(selSess, isSend, seq, random, msgTime, loginInfo.identifier, subType, loginInfo.identifierNick, loginInfo.headurl);
		msg.addText(new webim.Msg.Elem.Text(JSON.stringify(msgContent)));
		msg.originContent = msgContent;

		console.log("发送自定义消息给：", msgContent.toUserID, "消息类型：", msgContent.msgType)
		webim.sendMsg(msg, function(resp) {
		    console.log("发送自定义消息成功", resp)
            //发送成功，处理callbacklist
            callbackList.forEach(function(callback){
                invokeFunc(callback,  msgContent.msgData)
            })
